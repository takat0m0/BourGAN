...
def gaussian_mixture_lattice(batchsize, num_on_edge = 5, std = 0.2):
        rand_indices1 = np.random.randint(0, num_on_edge, size=batchsize)
        rand_indices2 = np.random.randint(0, num_on_edge, size=batchsize)
        mean = np.zeros((batchsize, 2), dtype=np.float32)
        mean[:, 0] = rand_indices1
        mean[:, 1] = rand_indices2
        return np.random.normal(mean, std**2, (batchsize, 2)).astype(np.float32)

if __name__ == '__main__':
        plot_scatter(gaussian_mixture_lattice(1000), dir = '.', color = 'red')        
